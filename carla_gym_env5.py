...
# Hilfsfunktion: Projektion von Weltkoordinaten in Bildkoordinaten (Top-Down-Vogelperspektive).
def world_to_birdview(world_loc, sensor_transform, image_width, image_height, meters_per_pixel=0.2):
    """
    Annahmen:
      - Die Kamera ist top-down: pitch=-90°, so dass X-Welt = +/- rechts, Y-Welt = +/- vorne/hinten,
        Z = Höhe. 
      - sensor_transform.location = (sx, sy, sz)
      - Wir konvertieren die (wx, wy) in relative Koordinaten zur Sensor-Position in der XY-Ebene,
        dann skalieren wir um in Pixel.
    Args:
        ...
        meters_per_pixel (float): Skalierung
    Returns:
        (u, v) in Bildkoordinaten oder None, wenn es außerhalb liegt.
    """
    sx, sy, sz = sensor_transform.location.x, sensor_transform.location.y, sensor_transform.location.z
    # Ignoriere Rotation hier vereinfacht, da Kamera pitch=-90, yaw=0: 
    # => +X in der Welt entspricht +X in Bild, +Y in der Welt => -Y in Bild (je nach Achsen)
    
    # relative Koordinaten
    dx = world_loc.x - sx
    dy = world_loc.y - sy

    # Da pitch=-90, "oben" ist +z => dx => horizontal, dy => vertikal
    # Wir nehmen an, +dy in der Welt soll nach "oben" in der Birdview gehen => also v = center - dy
    # Je nachdem, wie die Achsen orientiert sind, muss man ggf. invertieren.
    # Hier eine einfache Annahme:
...
class CarlaGymEnv(gym.Env):
    """
    Environment mit Birdview-Segmentation. Zusätzlich wird im Kamera-Callback
    der Bereich um den "nächsten Waypoint" (bzw. die 'korrekte Spur') farblich hervorgehoben.
    """
    ...
    def _init_vehicle_sensors(self):
        ...
        # Wir definieren hier den Callback mit Extra-Schritt zum Hervorheben:
        def camera_callback(image):
            ...
            # OPTIONAL: Hervorheben des nächsten Waypoints bzw. "korrekte Lane"
            # Indem wir die Pixel rund um die next_waypoint-Position mit z.B. label=0.8 einfärben:
            with self.image_lock:
                # Kopie erstellen, damit wir sie ggf. anpassen können
                labeled_img = labels.copy()
                self._highlight_correct_lane(labeled_img)
                # als (480,640,1) ablegen
    ...
    def _highlight_correct_lane(self, labeled_img):
        """
        Kleiner Trick: Markiere die Position des "nächsten Waypoints" farblich (z.B. label=0.8).
        Dabei wird im Birdview ein kleiner Kreis gezeichnet.
        """
        ...
        # Bestimme die Bildkoordinate (u,v) des next_waypoints
        wp_loc = self.next_waypoint.transform.location
        sensor_transform = self.camera_sensor.sensor.get_transform()

        # Wir nutzen "world_to_birdview" (vereinfachter Ansatz, ohne Rotation),
        # meters_per_pixel bestimmt den Zoom-Faktor.
        # Falls das Overlay an falscher Stelle erscheint, muss man
        # yaw etc. noch genauer berücksichtigen.
        ...
        # Zeichne einen kleinen Kreis (Radius ~ 5 Pixel) mit Wert=0.8:
        rr = 5
            ...
            for dx in range(-rr, rr+1):
                ...
                if 0 <= xx < 640 and 0 <= yy < 480:
                    ...
                    if dx*dx + dy*dy <= rr*rr:
                        labeled_img[yy, xx] = 0.8  # z.B. 0.8 => eigener "Lane-Highlight"
    ...
    def _compute_reward_done_info(self):
        ...
        #    Wir setzen den Threshold hier bewusst etwas kleiner (1.0), damit Ausreißer bestraft werden.
        ...
        # Wer weiter als 1 m weg ist, kriegt -1 und done.
        if offset_magnitude >= max_offset:
            print(">>> Zu weit von Fahrbahnmitte (möglicherweise Bordstein). Episode terminiert!")
            reward = -1.0
            done = True
        ...
        if lane_id_mismatch:
            ...
            # linear abnehmend von 0 -> -0.5, offset=0 => +0.5
    ...
    def _show_image(self, seg_img):
        ...
        cv2.imshow("CARLA Semantic Segmentation (BirdView, Lane-Highlight)", gray)
        cv2.waitKey(1)
